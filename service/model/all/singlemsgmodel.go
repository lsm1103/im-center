...
var (
	...
	cacheSingleMsgSeqSenderIdReceiverIdPrefix = "cache:singleMsg:seq:senderId:receiverId:"
)
...
type (
	SingleMsgModel interface {
		...
		FindOneBySeqSenderIdReceiverId(seq int64, senderId int64, receiverId int64) (*SingleMsg, error)
...
singleMsgSeqSenderIdReceiverIdKey := fmt.Sprintf("%s%v:%v:%v", cacheSingleMsgSeqSenderIdReceiverIdPrefix, data.Seq, data.SenderId, data.ReceiverId)
    ret, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", m.table, singleMsgRowsExpectAutoSet)
		return conn.Exec(query, data.Id, data.Seq, data.SenderType, data.SenderId, data.SenderDeviceId, data.ReceiverId, data.MsgType, data.Content, data.ParentId, data.SendTime, data.Status)
	}, singleMsgIdKey, singleMsgSeqSenderIdReceiverIdKey)
	return ret,err
}
*/
...
func (m *defaultSingleMsgModel) Insert(session sqlx.Session, data *SingleMsg) (sql.Result, error) {
	...
	ret, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", m.table, singleMsgRowsExpectAutoSet)
		if session != nil {
			return session.Exec(query, data.Id, data.Seq, data.SenderType, data.SenderId, data.SenderDeviceId, data.ReceiverId, data.MsgType, data.Content, data.ParentId, data.SendTime, data.Status)
		}
		return conn.Exec(query, data.Id, data.Seq, data.SenderType, data.SenderId, data.SenderDeviceId, data.ReceiverId, data.MsgType, data.Content, data.ParentId, data.SendTime, data.Status)
	}, singleMsgIdKey, singleMsgSeqSenderIdReceiverIdKey)
	return ret, err
...
func (m *defaultSingleMsgModel) FindOneBySeqSenderIdReceiverId(seq int64, senderId int64, receiverId int64) (*SingleMsg, error) {
	singleMsgSeqSenderIdReceiverIdKey := fmt.Sprintf("%s%v:%v:%v", cacheSingleMsgSeqSenderIdReceiverIdPrefix, seq, senderId, receiverId)
	var resp SingleMsg
	err := m.QueryRowIndex(&resp, singleMsgSeqSenderIdReceiverIdKey, m.formatPrimary, func(conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
		query := fmt.Sprintf("select %s from %s where `seq` = ? and `sender_id` = ? and `receiver_id` = ? limit 1", singleMsgRows, m.table)
...
singleMsgSeqSenderIdReceiverIdKey := fmt.Sprintf("%s%v:%v:%v", cacheSingleMsgSeqSenderIdReceiverIdPrefix, data.Seq, data.SenderId, data.ReceiverId)
    _, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Seq, data.SenderType, data.SenderId, data.SenderDeviceId, data.ReceiverId, data.MsgType, data.Content, data.ParentId, data.SendTime, data.Status, data.Id)
	}, singleMsgSeqSenderIdReceiverIdKey, singleMsgIdKey)
	return err
}
*/
...
func (m *defaultSingleMsgModel) Update(session sqlx.Session, data *SingleMsg) error {
	...
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, singleMsgRowsWithPlaceHolder)
		if session != nil {
			return session.Exec(query, data.Seq, data.SenderType, data.SenderId, data.SenderDeviceId, data.ReceiverId, data.MsgType, data.Content, data.ParentId, data.SendTime, data.Status, data.Id)
		}
		return conn.Exec(query, data.Seq, data.SenderType, data.SenderId, data.SenderDeviceId, data.ReceiverId, data.MsgType, data.Content, data.ParentId, data.SendTime, data.Status, data.Id)
	}, singleMsgSeqSenderIdReceiverIdKey, singleMsgIdKey)
	return err
